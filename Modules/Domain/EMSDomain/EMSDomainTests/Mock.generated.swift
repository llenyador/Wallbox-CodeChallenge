...
open class GetQuasarsEnergyResumeUseCaseProtocolMock: GetQuasarsEnergyResumeUseCaseProtocol, Mock {
    public init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        SwiftyMockyTestObserver.setup()
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
    ...
    open func execute() -> PublisherResult<QuasarsEnergyResume> {
        addInvocation(.m_execute)
		let perform = methodPerformValue(.m_execute) as? () -> Void
		perform?()
		var __value: PublisherResult<QuasarsEnergyResume>
		do {
		    __value = try methodReturnValue(.m_execute).casted()
		} catch {
			onFatalFailure("Stub return value not specified for execute(). Use given")
    ...
    open class Given: StubbedMethod {
        ...
        public static func execute(willReturn: PublisherResult<QuasarsEnergyResume>...) -> MethodStub {
            return Given(method: .m_execute, products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        public static func execute(willProduce: (Stubber<PublisherResult<QuasarsEnergyResume>>) -> Void) -> MethodStub {
            let willReturn: [PublisherResult<QuasarsEnergyResume>] = []
			let given: Given = { return Given(method: .m_execute, products: willReturn.map({ StubProduct.return($0 as Any) })) }()
			let stubber = given.stub(for: (PublisherResult<QuasarsEnergyResume>).self)
			willProduce(stubber)
			return given
        }
    }
